            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if( response.status == 'error' ){
                        server_result_display.empty().html('<div class="mt-3 alert alert-danger  alert-dismissible">'+ response.errors +' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                    }
                    else if( response.status == 'success' ){
                        server_result_display.empty().html('<div class="mt-3 alert alert-success  alert-dismissible">'+ response.message +'<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                        setTimeout(function(){
                ...
                error: function() {
                    server_result_display.empty().html('<div class="mt-3 alert alert-danger  alert-dismissible">Server error! Please try again...</div>');
                }
            });
        }
