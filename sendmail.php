<?php
    ...
    // Recaptcha settings
    $use_recaptcha = false;
    define("RECAPTCHA_V3_SECRET_KEY", '6LceXdwUAAAAAJxGDZxVUFyo5c33EYj0n6netovu');

    // Errors strings
    $first_name_error      = '*Invalid name (Only letters and white space allowed) <br>';
    $last_name_error      = '*Invalid name (Only letters and white space allowed) <br>';
        ...
        switch ($key){
            case 'first_name':
                if( !preg_match("/^[a-zA-Z ]*$/", $value) || empty($value) ) {
                    $serverErrors['errors'] .= $first_name_error;
                }
                else{
            ...
            case 'last_name':
                if( !preg_match("/^[a-zA-Z ]*$/", $value) || empty($value) ) {
                    $serverErrors['errors'] .= $last_name_error;
                }
                else{
                    $post_data .= "<strong>$key:</strong> $value <br>";
                }
